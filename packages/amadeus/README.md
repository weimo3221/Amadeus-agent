...
- `ContextAssembler` builds API-call-time context each turn: conversation summary, accepted structured memories, active session plan items, and current queued/running/blocked tasks go into the system message; relevant prior session snippets go into a sanitized `<memory-context>` block on the current user message; and a `memory.context.used` event reports which sources were used. Completed plan items and terminal tasks are not reinjected. These injected blocks are not persisted. The runtime keeps the most recent `context.diagnosticsLimit` diagnostics per session in an in-memory ring buffer for developer inspection.
