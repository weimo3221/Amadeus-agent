...
- `ContextAssembler` builds API-call-time context each turn from a runtime memory manager. Stable Markdown memory (`MEMORY.md` / `USER.md`) remains in the prompt layer, while the built-in local runtime memory provider exposes derived SQLite/session artifacts: conversation summary, accepted structured memories, and relevant FTS snippets. Active session plan items, todos, task state, and recent terminal task outcomes are runtime state, not durable memory. Relevant prior snippets go into a sanitized `<memory-context>` block on the current user message, external providers can add `<external-memory-context>`, and `memory.context.used` reports which sources were used. These injected blocks are not persisted. The runtime keeps the most recent `context.diagnosticsLimit` diagnostics per session in an in-memory ring buffer for developer inspection.
