...
Runtime memory/context/task tuning is loaded from `../../configs/runtime.yaml`. This file controls token-budget compaction, context assembler budgets, summary thresholds and cooldowns, the recent raw conversation window, background memory review limits, and worker approval expiry. The recent raw window is configured in user/assistant turns with `summary.keepRecentTurns` (default `3`) and `summary.minKeepRecentTurns`; attached assistant/tool messages for those turns are kept together. Worker action-specific approval expiry is configured with `tasks.workerApprovalActionTtlSeconds` (default `900`). The older `keepRecentMessages` names are still accepted as compatibility aliases. Environment variables remain the deployment override layer, so values such as `AMADEUS_CONTEXT_MAX_TOKENS`, `AMADEUS_SUMMARY_KEEP_RECENT_TURNS`, and `AMADEUS_WORKER_APPROVAL_ACTION_TTL_SECONDS` take precedence over the YAML file.
...
| `search_memory_items` | `allow` | Searches durable structured `user` / `agent` / `project` memory facts stored in SQLite, using BGE-M3 hybrid ranking when configured. |
...
- `ToolContext` carries session id, cwd, optional memory store, turn/tool-call ids, permission decision metadata, worker sandbox mode, audit metadata, session workspace epoch, timeout, cooperative cancellation, and model-output size limits.
...
- Task execution now goes through a `TaskRunner` boundary. `InProcessTaskRunner` preserves the current thread-pool behavior, while the state machine, retry/recovery logic, cancellation, and event publishing remain in `TaskWorker`. Worker runtime scopes carry a first sandbox mode: `read_only` hides/blocks mutation and execution tools, `workspace_write` allows workspace file mutation but blocks shell/process/code execution, and `workspace_execute` allows execution tools only when the task's profile/toolsets also allow them.
- `ContextAssembler` builds API-call-time context each turn from the active runtime memory provider. Stable Markdown memory (`MEMORY.md` / `USER.md`) remains in the prompt layer. `configs/runtime.yaml` defaults to `memory.provider: mem0_like_runtime`, which keeps the hybrid provider's summary/current-session FTS/global FTS lanes while durable long-term facts stay in typed `memory + metadata + history` records for tool-driven recall. Set `memory.provider: hybrid_runtime` for the previous provider, or `builtin_runtime` for the original local provider exactly as before. If an external provider is configured, it replaces the runtime provider for prefetch and memory-tool exposure so the model sees one memory backend. Active session plan items, todos, task state, and recent terminal task outcomes are runtime state, not durable memory. Relevant prior transcript snippets go into a sanitized `<memory-context>` block on the current user message, external provider results go into `<external-memory-context>`, and `memory.context.used` reports which sources were used, including `retrievalProvider` metadata. Structured `memory_items` are not injected automatically; the model must call `search_memory_items` when durable user, agent, or project facts may matter. These injected blocks are not persisted. The runtime keeps the most recent `context.diagnosticsLimit` diagnostics per session in an in-memory ring buffer for developer inspection.
...
- Structured `memory_items` store durable `user` / `agent` / `project` facts in SQLite using a Mem0-like shape: `content`, `memoryType`, JSON `metadata`, `contentHash`, source ids, access stats, timestamps, and soft deletion. They maintain a dedicated `memory_items_fts` BM25 index over content/type/metadata, expose explicit HTTP add/list/delete/history APIs, are searchable through the read-only `search_memory_items` tool with optional metadata filters, can be added/replaced/forgotten through approval-gated tools, and are no longer injected into every turn as `<memory-items>`.
- Memory query tokenization uses `jieba` plus bounded CJK n-gram fallback. `messages_fts` indexes the original message text plus tokenized terms so Chinese queries can match Chinese historical messages by segmented terms; search results still return the original message content. Structured `memory_items` use the same tokenizer in `memory_items_fts`, so durable fact retrieval uses BM25 over content and metadata instead of only SQL LIKE fallback.
...
- `mem0_like_runtime` is the default provider. It keeps summary/current-session FTS/global FTS context prefetch behavior while long-term memory remains typed items with metadata, access stats, history, and optional BGE-M3 vector embeddings for tool-driven recall.
...
- `memory.vectorRetrieval: true` lets `search_memory_items` use local BGE-M3 dense vectors for typed long-term memory when the embedding provider is configured and deployed. If BGE-M3 is unavailable, the tool automatically falls back to the `memory_items_fts` BM25 path.
- `memory.vectorCandidateLimit` bounds the durable memory item candidate set used by tool-driven hybrid vector/BM25/metadata ranking.
...
- `POST /memory/items`：add a typed long-term memory item; accepts optional `memoryType` and `metadata`. `GET /memory/items` supports `scope`, `memoryType`, `query`, `metadataFilter=<json>`, and `metadata.<key>=value` filters.
