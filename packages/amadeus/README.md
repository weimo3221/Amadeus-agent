...
- Task execution goes through a `TaskRunner` boundary. Production uses an independent SQLite-leased durable supervisor and subprocess runner; `TaskWorker` remains the state-machine owner. Worker runtime scopes carry `read_only`, `workspace_write`, or `workspace_execute` tool policy, workspace-copy isolation keeps task files separate, and `AMADEUS_TASK_OS_SANDBOX` adds a probed bubblewrap/Seatbelt process boundary with explicit `auto`, fail-closed `required`, and compatibility `none` modes.
