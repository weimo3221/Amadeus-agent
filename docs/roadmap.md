...
- Core Memory v2 mechanics are now implemented: SQLite FTS retrieval, stable Markdown memory, Mem0-like structured memory facts, explicit memory tools, review candidates, accept/reject flows, automatic review gates, runtime memory config, item history/access metadata, and memory safety filters.
