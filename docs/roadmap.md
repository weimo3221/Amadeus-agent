...
- Worker ask-tool approval checkpoints now prefer action-specific authorization: terminal exact command keys, process action keys, and path/target keys are saved with risk metadata and a default 15-minute expiry, then resume into one-run `WorkerRuntimeScope` action approvals; tool-wide resume approval remains only for older checkpoints without an action key.
...
- External durable multi-process task supervisor, inline approval/resume copy, configurable expiry/classifiers for risky worker actions, more granular file-resume overrides, and truly isolated child-runtime workspace sandboxing.
