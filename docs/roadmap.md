...
- External durable multi-process task supervisor, tool/state-level continuation from saved phases, broader approval checkpoint policy for risky worker actions, and truly isolated child-runtime workspace sandboxing.
...
- Long task plans.
- Human approval checkpoint policy for destructive, sensitive, or low-confidence worker actions beyond the current review-completion checkpoint.
