...
- First task runner abstraction complete: `TaskRunner` / `InProcessTaskRunner` separate scheduling/execution submission from the task state machine. A first optional POSIX `ProcessTaskRunner` is available behind the same contract, and `amadeus.task_worker_entrypoint` can run one claimed task from a dedicated process using `AMADEUS_TASK_ID` plus a SQLite database path.
...
- External durable multi-process task supervisor with subprocess launch, restart/reclaim policy, and worker-profile/env binding.
