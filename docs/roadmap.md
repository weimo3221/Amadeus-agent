...
1. Current: continue hardening Vue Main UI as the primary workbench. Companion should stay lightweight, transparent, Live2D-focused, voice-capable, and transient; Main UI should own larger chat, session history, active/completed tasks, timed messages, skills, memory review, diagnostics, permissions, model/runtime configuration, Live2D/TTS configuration, MCP server management, and Role-scoped runtime selection.
   - `apps/desktop-ui-next` is the replacement for the legacy vanilla Main UI renderer, not for the whole `apps/desktop` Electron shell. The cleanup target is `apps/desktop/src/renderer/main-ui` plus its legacy build/test assumptions after Vue-based Electron E2E coverage is complete.
2. Current: make the task system heavier without adding competing task concepts. Plans describe turn-local intent/progress, scheduled jobs trigger work, and tasks remain the durable execution unit for retry/cancel/recovery/results. Plan item -> Task, scheduled agent-task triggers, task detail timeline, cancel, re-run, Hermes-style turn-scoped plan display, persisted plan-run snapshots, blocked/review controls, normalized typed artifacts, task notifications, worker leases, periodic lease heartbeats, restart recovery for expired leases, an optional POSIX process runner, a single-task process worker entrypoint, a subprocess launcher/supervisor runner with first-pass non-zero exit reclaim plus abandoned-attempt marking, first-pass `WorkerRuntimeScope` tool/workspace/permission/sandbox policy, stage-oriented worker attempt checkpoints, checkpoint-aware retry handoff, resume-strategy worker prompting, Main UI checkpoint/resume diagnostics, review approval checkpoints, worker ask-tool approval checkpoints, first worker action-specific approval keys with default expiry, first destructive/sensitive action classifiers, task-detail approval action/risk/expiry UI plus inline approve/resume copy, first saved-phase continuation prompts, worker tool-result artifacts, file-state/idempotent resume metadata, file hash manifests, manifest verification, first `fileResumePolicy` instructions and overrides, task-detail policy UI exposure, first runtime guardrail enforcement for verified file resume policies, subprocess workspace-copy isolation, child runtime workspace-root override, worker execution-tool workspace sandbox env/guards, stricter high-risk worker auto-approval blocking, and worker isolation/policy contract evals are now wired; the next concrete step is richer artifact producers plus durable supervisor and stronger OS-native sandbox hardening.
...
- First task runner abstraction complete: `TaskRunner` / `InProcessTaskRunner` separate scheduling/execution submission from the task state machine. A first optional POSIX `ProcessTaskRunner` is available behind the same contract, `amadeus.task_worker_entrypoint` can run one claimed task from a dedicated process using `AMADEUS_TASK_ID` plus a SQLite database path, and `SubprocessTaskRunner` can launch that entrypoint through `AMADEUS_TASK_RUNNER=subprocess` with run/profile env binding, first-pass non-zero exit reclaim, and `abandoned` attempt marking. `AMADEUS_TASK_WORKSPACE_ISOLATION=copy` now creates a per-run workspace copy under the worker sandbox root, passes `AMADEUS_WORKER_WORKSPACE_OVERRIDE` / isolation / source metadata into the child process, and makes the child `AgentRuntime` and `MessageMemoryStore` use that isolated workspace as their root while validating that the source workspace still belongs to the session workspace. Worker profile/toolset/workspace/permission policy now narrows child task tool schemas, execution checks, ToolContext metadata, audit metadata, workspace root resolution, and ask-tool behavior through `WorkerRuntimeScope`; non-copy workspace hints are rejected before model execution unless they resolve inside the current session workspace. Non-auto-approved worker ask-tools now block the task with a tool-specific approval checkpoint instead of opening interactive prompts or failing outright; resumed checkpoints can authorize that ask-tool for the next worker run. Worker attempts now heartbeat stage-oriented checkpoints so recovery can see the last phase, last event, worker profile/toolsets, and output/error previews; worker tool results are stored as typed task artifacts and rehydrated into later WorkerContext prompts as `<task-artifacts>`; patch/write/read/search/terminal previews now carry first-pass resume metadata such as affected files, observed files, commands, idempotency hints, and file manifests with size/mtime/SHA-256 where available; `WorkerContext` verifies saved file manifests against current workspace state and exposes unchanged/changed/unverifiable results plus file resume policies such as `skip_redundant_mutation` and `reinspect_before_mutation`; Main UI task details now expose those verified file resume policies on artifact cards; stale recovery and subprocess-loss retry promote checkpoint handoff context for the next worker; WorkerContext renders phase-specific `<resume-strategy>` instructions, approval resume prompts constrain the approved tool to the blocked step, Main UI task details expose the checkpoint/resume handoff state for auditing, and review-required completions persist approval/resume checkpoints.
- Worker ask-tool approval checkpoints now prefer action-specific authorization: terminal exact command keys, process action keys, and path/target keys are saved with risk metadata and configurable expiry from runtime config, then resume into one-run `WorkerRuntimeScope` action approvals; legacy tool-wide resume approval remains only for older checkpoints without an action key and no longer bypasses high-risk or blocking-label actions.
...
- External durable multi-process task supervisor, richer OS-native sandbox backends beyond the current copy/env/guard layer, and override audit/policy tuning.
