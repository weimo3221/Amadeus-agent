...
2. Current: make the task system heavier without adding competing task concepts. Plans describe turn-local intent/progress, scheduled jobs trigger work, and tasks remain the durable execution unit for retry/cancel/recovery/results. The task system now includes plan/task linkage, task details and controls, typed artifacts, leases and heartbeats, checkpoint-aware retry/resume, review and action-specific approval checkpoints, file-state resume policies, worker sandbox modes, subprocess workspace-copy isolation, a production-default subprocess runner, duplicate launch suppression, process-group cancellation, lifecycle audit events, runtime health diagnostics, and periodic expired-lease recovery/queued dispatch. The next concrete step is richer artifact producers, failure-injection evals, independently deployable supervisor hardening, resource controls, and stronger OS-native sandbox backends.
...
- First task runner abstraction and production-default subprocess slice complete: `TaskRunner` separates scheduling/execution submission from the task state machine; in-process, synchronous, and optional POSIX fork runners remain available, while server startup defaults to `SubprocessTaskRunner` plus workspace-copy isolation. The subprocess path binds run/profile/workspace env, suppresses duplicate active launches, uses independent process groups, supports task-specific termination, records lifecycle events, reclaims non-zero exits, and marks interrupted attempts `abandoned`. `TaskWorker` periodically re-runs expired-lease recovery and queued dispatch, `/runtime/health` exposes supervisor state, and SIGTERM shuts down scheduler/worker children. Worker scope, approval, checkpoint, artifact, file-manifest, and resume-policy behavior remains shared across runners.
...
- External durable multi-process task supervisor, multi-child graph orchestration quality, richer OS-native sandbox backends beyond the current copy/env/guard layer, and override audit/policy tuning.
...
- Runtime contract eval now runs nine structured scenarios through `npm test`, adding the tracked isolated child-agent/session/summary-handoff path to role/task context, cross-runner lifecycle consistency, orchestrator repair/dependency/artifact handoff, MCP, subprocess fault recovery/cancellation/deduplication, workspace/symlink/path/write sandbox boundaries, and action-specific approval expiry/scope. Broaden next into model-backed planning quality, longer crash/restart soak tests, Memory, Live2D/audio, and richer tool-choice/guardrail behavior.
