...
- Own the internal long-task orchestrator control plane. The current deterministic `OrchestratorService` validates structured task graphs, creates child tasks and dependency edges, dispatches ready children, and reviews terminal child status; model-backed graph generation remains a later layer on top of this internal service.
