...
- `memory_provider.py`: runtime memory provider layer. The built-in local provider exposes derived session memory artifacts such as summaries, accepted structured facts, and FTS snippets; optional external providers contribute bounded reference snippets without becoming durable local memory or overriding user/system instructions. Raw session transcripts remain in `memory.py` and are read through explicit transcript tools/APIs.
