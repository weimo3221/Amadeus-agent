...
- `context.py`: active API-call-time context assembler for memory-provider output, active todos, task state, source budgets, and diagnostics. Stable Markdown memory stays in the prompt layer. With the default `mem0_like_runtime` provider active, SQLite/session-derived context includes conversation summaries and relevant transcript FTS retrieval; typed long-term `memory_items` are not automatically injected and are retrieved through `search_memory_items` when needed. Per-turn reference context such as active plans, todos, tasks, recent task outcomes, FTS snippets, and external memory is appended to the current user message as non-persistent reference data. `AgentRuntime` keeps recent context diagnostics per session in an in-memory ring buffer.
...
- `memory_provider.py`: runtime memory provider layer. Exactly one provider is active at a time. The default `mem0_like_runtime` local provider exposes summary/FTS prefetch artifacts and the SQLite memory tools; typed long-term memory items are recalled explicitly through `search_memory_items`, which can use BGE-M3 vector + BM25 + metadata-aware hybrid ranking. `hybrid_runtime` and `builtin_runtime` remain compatibility options. An external provider replaces that runtime prefetch/tool surface when configured. Raw session transcripts remain in `memory.py` and are read through explicit transcript tools/APIs.
...
- `tools/`: active concrete Python tool implementations and public registry entrypoint, including workspace file/memory tools, terminal/process execution, web search/extract, disabled-by-default browser bridge tools, vision endpoint bridge, clarify, and bounded Python code execution.
...
- `skills.py`: active reusable behavior boundary; owns skill discovery, metadata/view APIs, filtered skill catalog prompt assembly, explicit turn-level skill prompt injection, and approved experience-skill creation. The project skill tree now includes `skills/web-access`, which supplies a CDP-backed browser procedure for real web access while leaving the built-in `web_search` / `web_extract` tools as lightweight static tools.
...
- Own runtime skill activation for the preferred path. Skills are procedural context rather than concrete tools: a model can call `skill_view("web-access")` to load the CDP browser workflow, then use existing tools such as `terminal` under normal permission and audit controls to run the skill's helper scripts.
- Own persisted session tasks as the execution unit for longer-running work, with task metadata (`kind`, `source`, `parentTaskId`, `rootTaskId`, `planRunId`, `planItemId`, `workerType`, `workerProfile`, acceptance criteria, context hints, toolset bounds, review/artifact fields), independently supervised subprocess/copy-isolated production execution, optional fail-closed bubblewrap/Seatbelt containment, retry scheduling, stale-lease recovery, durable process adoption, blocked review gates, dependency-aware runnable selection, typed artifacts, and attempt history. Tasks can be linked to visible plan items through `planItemId`; worker lifecycle updates move linked plan items to `in_progress`, `completed`, `pending`, or `cancelled`. Current workers build an isolated `WorkerContext` from the task spec, dependency artifacts, and previous attempts instead of replaying the parent conversation.
- Own the internal long-task orchestrator control plane. `OrchestratorService` validates structured task graphs, marks the root as orchestrator-owned waiting state before creating children, enforces per-graph concurrency in both discovery and atomic claim, dispatches dependency-ready children, cascades root cancellation, performs bounded append-only failed-child replacement with atomic dependency rewiring, and synthesizes effective terminal children. Model-backed graph generation remains a controlled layer on top of this service.
...
- Mature long-running execution beyond the current single-host SQLite-leased supervisor and first bubblewrap/Seatbelt boundary with broader platform/backend deployment coverage, longer double-crash/restart soak coverage, resource-usage telemetry, and richer user-facing notification policy.
