...
- Own persisted session tasks as the execution unit for longer-running work, with task metadata (`kind`, `source`, `parentTaskId`, `rootTaskId`, `planRunId`, `planItemId`, `workerType`, `workerProfile`, acceptance criteria, context hints, toolset bounds, review/artifact fields), subprocess/copy-isolated production execution, retry scheduling, periodic stale-lease recovery, blocked review gates, dependency-aware runnable selection, typed artifacts, and attempt history. Tasks can be linked to visible plan items through `planItemId`; worker lifecycle updates move linked plan items to `in_progress`, `completed`, `pending`, or `cancelled`. Current workers build an isolated `WorkerContext` from the task spec, dependency artifacts, and previous attempts instead of replaying the parent conversation.
...
- Mature long-running execution beyond the current runtime-owned subprocess supervisor into an independently deployable supervisor, stronger resource controls, OS-native sandbox backends, and richer user-facing notification policy.
