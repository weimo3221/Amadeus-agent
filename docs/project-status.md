...
Amadeus is now a working desktop MVP with a Python-first turn path, split Electron desktop surfaces, local ASR/TTS voice I/O, scheduled triggers, persistent session todos, tracked background tasks, an independent durable task supervisor, and a mostly landed runtime reliability foundation.

The current project phase is no longer initial MVP construction. The main MVP surfaces are present, Python owns the preferred runtime path, ToolRuntime is in late-stage hardening, Memory v2 has its core storage/review/context pieces in place, and the first Live2D/audio harness slices are active. The desktop product surface now has separate `companion` and `main-ui` entries: Companion owns Live2D, voice input, and lightweight desktop presence, while the Vue `desktop-ui-next` Main UI owns the larger workbench/chat surface. Main UI restores current session history, displays active and completed timed messages, exposes richer runtime configuration, and uses a light anime plus modern SaaS visual treatment. The next large product step is desktop/runtime stabilization: polish CLI/session switching, refine the two desktop surfaces, improve lipsync/ASR quality, continue TypeScript bridge shrinkage, and harden ToolRuntime/Memory only where real usage exposes gaps.
...
- Desktop app MVP is running with Electron and Vite renderer entries. `companion` is a transparent frameless desktop presence with Live2D, lightweight text/voice input, transient streaming reply bubbles, runtime audio playback, and a hybrid lipsync path: provider-native or runtime-planned phoneme/viseme lipsync cues when available, desktop amplitude-driven mouth movement for runtime audio otherwise, and the older timed mouth loop kept as fallback. `desktop-ui-next` is the sole larger chat/workbench renderer without Live2D.
...
- `scripts/amadeus_cli.py` is now the first real command-line client. It calls the Python runtime directly, defaults to the independent `cli:default` session, can send `/agent/turn` requests with explicit skills, resolves streamed `tool.permission.request` events through `/tools/permission`, and exposes `doctor`, `skills`, `memory`, and `speak` subcommands over the same Skills/MCP/Memory/Audio HTTP surfaces used by the desktop.
...
- Main UI task management now includes a task detail modal with execution metadata, source/relationship labels, result/error/typed-artifact display, worker lease and checkpoint/resume diagnostics, verified file resume policy display on artifact cards, `/tasks/{id}/events` timeline, cancellation for queued/running tasks, review approval / resume controls for blocked tasks, and re-run for terminal tasks through a new linked background task. Workspace overview shows recent terminal/blocked task notifications that link back to the Tasks view.
- Main UI includes a configuration center for model provider/API settings, model thinking mode and reasoning effort (`low` / `medium` / `high`), Live2D model import/selection/behavior mapping, macOS/GPT-SoVITS TTS settings, and runtime config persistence through Python endpoints. The configuration center now opens with a Runtime Doctor tab backed by Python `GET /runtime/health`; it summarizes runtime/model/memory/task worker/embedding/tools/Live2D/audio/config state, highlights first-start blockers such as missing API keys or unavailable local resources, and links directly to the relevant model, memory, Live2D, or voice configuration tab.
...
- `delegate_task` now creates a tracked `delegated` task and executes it through the normal isolated TaskWorker path. The research child uses a task-specific archived session, `WorkerContext`, read-only `WorkerRuntimeScope`, bounded source-session memory/file search, no write/shell/task/delegation tools, parent cancellation/timeout propagation, and summary-only handoff back to the parent agent.
...
- Runtime observability first slice is in place.
  - Python exposes `GET /runtime/observability?sessionId=...` as a read-only cross-module snapshot over runtime health, recent tasks and latest task graph, role-scoped Skills, effective ToolRuntime inventory, MCP/tool audit failures, and recent `memory.context.used` diagnostics.
  - The Vue Main UI now includes a dedicated Run Observability page that summarizes health, active/blocked tasks, tool/MCP failures, Memory source counts, role scope, MCP server state, and recent failures while linking conceptually back to the deeper task, memory, skills, MCP, and config pages.
- P3 Release hardening first slice is in place.
  - `scripts/release_check.py` validates release scripts, desktop release build wiring, packaged-real-runtime E2E coverage, runtime soak wiring, documentation, and the Electron auto-update bootstrap.
  - `scripts/runtime_soak.py` can poll an already-running Python runtime and Node bridge for a configurable duration through `/runtime/health`, `/runtime/observability`, and `/health`, emitting a JSON pass/fail summary for local or CI soak runs.
  - `apps/desktop/src/main/auto-update.ts` wires `electron-updater` behind `AMADEUS_AUTO_UPDATE=1` and `AMADEUS_UPDATE_FEED_URL`; it is packaged-only and does not make update network calls during dev or E2E.
...
- Latest P2 Runtime Observability validation passes: `npm run typecheck`, `npm run build:ui-next`, focused `python -m unittest tests.test_python_runtime_http tests.test_tool_runtime`, and full `npm test` with 466 Python tests (2 skipped), 35 Server tests, 21 Desktop tests, and 14/14 runtime contracts.
- Latest P3 Release hardening validation passes: full `npm run release:preflight`, covering all workspace typechecks, desktop release build, full `npm test` with 468 Python tests (2 skipped), 35 Server tests, 21 Desktop tests, 14/14 runtime contracts, 11/11 Electron E2E scenarios, and `release:check`.
- `apps/desktop-ui-next` is the sole production Main UI workbench. The Vue workbench connects to the live WebSocket bridge and Python HTTP runtime for chat, session switching, inline session title rename with immediate local title sync, Companion attach/view, turn-scoped plans, task details, timed messages, skills, memory diagnostics, runtime observability, MCP diagnostics, role-scoped runtime selection, and model/Live2D/TTS configuration.
...
- Continue consolidating the desktop UI now that Main UI and Companion are split. Companion should stay lightweight and Live2D/voice-focused; the Vue Main UI should remain the single production workbench for richer context, session switching, Companion attach/view, task details, permissions, memory, MCP, and configuration. Avoid adding parallel workbench surfaces.
- Continue P0 productization beyond the first Runtime Doctor UI slice: move API keys from plain `.env` editing toward OS-backed secure storage, add a real first-run wizard, and keep the runtime health surface aligned with startup/dependency failures.
- Continue CLI product hardening beyond the first HTTP client slice: richer TTY interaction, install/packaging affordances, transcript display controls, and optional WebSocket session attachment for watching shared desktop sessions.
...
- Test coverage now includes Python runtime units, local Python HTTP handlers, TypeScript bridge relay behavior, server-level WebSocket integration behavior, desktop renderer runtime UI behavior, packaged Vue Main UI interactions, and Companion Live2D/audio/hover smoke flows. Deeper E2E with real Live2D assets and live runtime providers is still missing.
...
Started: the storage/API/UI foundation and session-scoped task worker are in place, with worker retry, periodic stale-lease recovery, durable task graphs, dependency-aware and per-graph concurrency-bounded runnable selection, isolated WorkerContext prompts, model-backed graph decomposition/repair/lifecycle events/root synthesis, bounded failed-child replacement, root-owned cancellation, profile/toolset policy validation, an optional POSIX process runner, a single-task worker process entrypoint, and a production-default independently supervised subprocess/copy-isolated runner with process adoption, resource limits, process-group cancellation, lifecycle audit events, non-zero exit reclaim, and abandoned-attempt marking. Autonomous scheduling, richer graph UI semantics, deeper model-driven repair/eval loops, and long double-crash/restart soak coverage are not done.
...
- Added task graph HTTP APIs: `GET /tasks/{id}/graph`, `GET /tasks/{id}/attempts`, `GET /tasks/{id}/artifacts`, controlled `POST /tasks/{id}/decompose` for applying a validated structured graph with bounded width/replan policies, `POST /tasks/{id}/dispatch` for submitting dependency-ready child tasks, `POST /tasks/{id}/replan` for one bounded replacement of a failed child, and `POST /tasks/{id}/synthesize` for root task synthesis after children finish.
...
- Added the first isolated `WorkerContext` builder: in-process task workers now prompt the agent with task spec, acceptance criteria, root task summary, dependency artifacts, context hints, toolset bounds, previous attempt history, and checkpoint-derived resume strategy instead of using only task title/body or replaying the parent conversation.
- Worker model turns now use archived `worker:<taskId>` sessions rather than the parent session, so child prompts, tool messages, and assistant output do not enter parent raw history. Worker turns disable automatic stable-memory, transcript, global-FTS, and external-memory prefetch; allowed memory tools are explicit, and transcript search/read is bounded to the worker session or recorded source session.
- Task worker executions now create `task_attempts`, heartbeat the active attempt with stage-oriented checkpoint state, finish attempts with result/error/checkpoint state, and write successful worker results as summary artifacts for downstream dependency handoff.
- Added process-backed production execution: `TaskWorker` runs behind a `TaskRunner` contract, production defaults to `AMADEUS_TASK_RUNNER=subprocess`, `AMADEUS_TASK_SUPERVISOR_MODE=external`, and workspace-copy isolation; `in_process`, embedded subprocess supervision, and the optional supported POSIX fork runner remain explicit compatibility/test choices. `packages/amadeus/task_supervisor.py` is now the production task control process. It competes for a SQLite single-primary lease, dispatches queued work through the existing task state machine, and survives Python HTTP runtime restarts independently.
- Added durable subprocess control state: SQLite `supervisor_leases` and `task_processes` record supervisor ownership, worker PID/process group, run/task identity, workspace, log path, heartbeat, exit state, and metadata. A restarted supervisor adopts live worker PIDs, reclaims missing workers through normal retry/fail semantics, terminates cancelled or wall-time-expired adopted workers with TERM-to-KILL escalation, and writes per-run combined stdout/stderr logs. Worker entrypoints apply auditable wall-clock, CPU, address-space, file-size, and open-file limits where the host supports POSIX `setrlimit`.
- The Python runtime uses `ExternalSupervisorTaskRunner` as a durable DB-backed client in external mode; `/runtime/health` reports configured runner, supervisor mode, lease liveness, and active process registry, degrading when the external supervisor is absent. `scripts/dev_stack.py` starts the task supervisor before the runtime. The previous embedded recovery loop remains available through `AMADEUS_TASK_SUPERVISOR_MODE=embedded`.
- Added the OS-native worker sandbox boundary. `AMADEUS_TASK_OS_SANDBOX=auto` probes Linux bubblewrap or macOS Seatbelt before launch; `required` fails closed, explicit backend names require that backend, and `none` is an intentional compatibility mode. Bubblewrap gives workers a read-only host view, writable SQLite state, read-only shared sandbox root, and a writable current workspace. Seatbelt allows writes only to the current workspace and SQLite database sidecars. The selected backend, enforcement flag, executable, and fallback reason are durable process/event/lease metadata and appear in runtime health.
- Worker `workerProfile`, `allowedToolsets`, `disallowedTools`, task workspace hints, and task sandbox mode narrow the child turn's available tool schemas, prompt hints, execution checks, ToolContext metadata, audit metadata, and workspace root resolution through an `AgentRuntime` `WorkerRuntimeScope`; workspace hints outside the current session workspace fail before model execution with a `worker_scope_invalid` attempt checkpoint, while `read_only`, `workspace_write`, and `workspace_execute` sandbox modes hide or block mutation/execution tools before handler execution with `worker_sandbox_denied` audit records. Worker ask-tool policy is profile-aware: approved tools such as researcher `web_extract` and coder `patch` can run with `worker_auto_approved`, while other ask tools block the task with a tool-specific approval checkpoint instead of opening interactive prompts or failing outright. Attempt checkpoints record context build, scope validation, model turn, tool, assistant output, error, cancellation, review, approval, and completion phases. Tool result artifacts carry file/command state, idempotency hints, manifests, verification, and resume policy; recovery promotes the latest attempt checkpoint into task-level `resumeFrom` and handoff context.
- Worker ask-tool approval is now action-specific when the blocked event provides action metadata: terminal exact command keys, process kill/status keys scoped to PID and signal, process-list keys scoped to a normalized query hash, and path/target keys resume as one-run `approvedToolActions` with `approvedToolActionExpiresAt`, while old tool-wide approval remains only as a legacy fallback for checkpoints without an action key.
- Worker action classifiers now mark first-pass destructive/sensitive/low-confidence categories on approval checkpoints, including destructive or privileged shell commands, installers, network scripts/access, secret-like output, sensitive paths, workspace-external paths, whole-file writes, bulk replacements, insecure/sensitive URLs, and unknown targets.
- Approval decisions now have a correlated durable audit trail: blocked task events preserve tool/action/risk metadata; resumed events preserve action or legacy-tool scope, TTL/expiry, source, and actor; review approval events preserve task-review source/actor; successful `worker_auto_approved` tool audit records preserve the concrete action key/label, risk labels, and approval expiry.
- Subprocess workers now have a first real isolated child-runtime slice: `AMADEUS_TASK_WORKSPACE_ISOLATION=copy` copies the workspace into a per-run sandbox root, respects task workspace subdirectory hints, skips generated/heavy directories and external symlinks, passes `AMADEUS_WORKER_WORKSPACE_OVERRIDE` / isolation / source metadata into the child, and makes `task_worker_entrypoint` construct both `MessageMemoryStore` and `AgentRuntime` against that isolated root. Copy-isolated worker roots may live outside the session workspace, but validation requires the recorded source workspace to remain inside the session workspace. `ToolContext` and tool audit metadata now include workspace isolation/source fields. Worker `terminal` and `execute_code` calls in `workspace_execute` scope use workspace-local `HOME`/temp/Python path state; terminal rejects obvious absolute/relative outside-workspace path references, and Python execution injects a `sitecustomize` write guard that blocks common file writes outside the worker workspace.
...
- Multi-child graph ownership is now explicit. Applying a graph moves the root to `blocked/orchestrator_waiting` before child creation so the durable supervisor cannot execute the root as an ordinary worker. `maxConcurrency` is enforced both when listing runnable children and inside the SQLite `BEGIN IMMEDIATE` claim transaction. Root cancellation cancels active children before the root; direct child cancellation remains local to that child.
- Added bounded append-only failed-child replan. `maxReplans` limits graph-level replacement count, one failed node can be replaced once, the replacement inherits the original worker/tool policy, upstream dependencies are copied, queued/blocked downstream edges are atomically rewired, and synthesis ignores only explicitly superseded failed nodes. `graph.replanned` and `graph.cancelled` preserve the control history.
...
- Add long double-crash/restart and orphan-adoption soak coverage around the independent local supervisor; distributed or cross-host scheduling remains out of scope for the current SQLite control plane.

### Phase 12: Advanced Agent Features

Started: restricted `delegate_task` now runs a real tracked isolated child model loop through TaskWorker, the Hermes-style prompt surface split is in place, and the first bounded multi-child graph control flow is complete. Richer graph UI, model-driven repair quality, and long durable restart/soak validation are not done.
...
- The task detail modal now surfaces retry/run timing, heartbeat/finish timing, worker lease state, checkpoint phase/reason, `resumeFrom` hints, handoff summaries, localized event labels, typed artifacts, verified file resume policies, and existing cancel/rerun/approve/resume actions in one place, making task recovery and review flows easier to audit from Main UI.
- Added first human approval checkpoint slice: review-required worker output now persists a task-level `approval_required` checkpoint plus handoff summary while blocked, approval writes an `approved` checkpoint, and worker ask-tools outside the profile auto-approve set now block with a tool/action-specific approval checkpoint. Manual resume records either `approval_resume_requested` or `blocked_resume_requested`; when a tool action was approved, the resumed `WorkerRuntimeScope` carries that action key as a one-run approval and WorkerContext emits a constrained resume strategy that says to use only that approved command/action/path/target for the blocked step. Main UI task details render the approval action key/label, risk level/labels, tool name, expiry countdown/status, scoped approval explanation, and approval-aware resume button text so users do not need to inspect raw checkpoint JSON.
- Added first tool-output/file-state continuation slice: worker `tool.finished` previews are saved as normalized task artifacts with tool metadata, current-task artifacts are included in WorkerContext, and patch/write/read/search/terminal artifacts expose resumeKind, affected/observed files, command state, idempotency hints, file manifests with size/mtime/SHA-256, current manifest verification, and `fileResumePolicy` instructions so retry/resume prompts can skip redundant mutations or reinspect changed files before repeating work. Main UI task details now render those policies as auditable artifact card callouts with status, instructions, and override tags. Worker runtime scopes now also carry verified file resume policies into `ToolLoopGuardrail`: unchanged mutation artifacts can block same-tool repeat patch/write calls before handler execution, changed mutation artifacts require a same-turn `read_file` refresh before another mutation of the affected path, and `fileResumePolicy.override` can explicitly allow `force_rerun`, `ignore_artifact`, or `accept_current_state`. Task artifact detail data now loads from the first-class artifact endpoint, and Main UI can set or clear those overrides through `POST /tasks/{taskId}/artifacts/{artifactId}/file-resume-override`, refreshing the task artifact list and publishing `artifact_override_updated`; each set/clear operation also records a durable task event with artifact path, policy action/paths, previous/new override, change status, source, and actor, and the task timeline has dedicated labels for both event types.
- Added the task runner abstraction and promoted the subprocess runner to the production default. `TaskWorker` delegates execution to `TaskRunner`; `InProcessTaskRunner`, `SynchronousTaskRunner`, and optional POSIX `ProcessTaskRunner` remain available for compatibility/tests. The single-task entrypoint plus `SubprocessTaskRunner` now provide run/profile/workspace-isolation env binding, copied per-run workspaces, duplicate launch suppression, process-group cancellation, lifecycle audit events, periodic lease recovery/dispatch, non-zero exit reclaim, and `abandoned` attempt marking.
...
- Expanded `scripts/eval_runtime_contracts.py` into a fourteen-scenario deterministic suite with structured JSON and per-check metrics output: role/task context, task lifecycle, cross-runner contract matrix, isolated child-agent/session/summary handoff, orchestrator repair/dependency/artifact handoff, bounded graph quality/replan, MCP execution, subprocess duplicate/cancel/non-zero-exit/periodic-recovery faults, durable supervisor restart/adoption/lost-process soak, workspace-copy/symlink/path/write sandbox boundaries, Memory BM25 recall and safety policy, Audio fallback/lipsync timing, action-specific approval target/expiry scope, and durable approval/override audit metadata. `npm test` runs this suite through `npm run test:contracts`. The matrix also exposed that the full threaded runtime can crash after raw `fork` on macOS, so `ProcessTaskRunner` is explicitly disabled there in favor of `SubprocessTaskRunner`.
- Added `npm run cli` / `scripts/amadeus_cli.py` as a direct Python-runtime CLI: `ask` streams `/agent/turn` and handles permission prompts, `doctor` summarizes runtime/model/Skills/MCP/Memory/Audio state, `skills` lists or views skill instructions, `memory` inspects context diagnostics and typed memory search, and `speak` exercises `/audio/speak` fallback/provider behavior. The default session is `cli:default` so command-line use does not silently write into the desktop default session.
...
- Add richer user-facing graph orchestration controls and observability.
- Add context compression.
- Add model-driven replan strategies, broader approval/override policy tuning, wider native-sandbox deployment coverage, and UI-facing graph observability on top of the tracked isolated child runner, bounded multi-child graph control plane, WorkerContext builder, subprocess launcher, copy-isolated runtime, bubblewrap/Seatbelt boundary, first-pass `WorkerRuntimeScope`, checkpoint-aware retry handoff, approval checkpoints, resume prompts, sandbox modes, worker tool-output/file-state artifacts, durable approval/override audit trail, and fourteen-scenario deterministic quality eval.
- Continue refining approval checkpoints with richer policy coverage and UI copy for destructive/sensitive/low-confidence actions beyond the current target-scoped classifier and audit set.
- Add provider/harness profiles.
- Add broader eval coverage for model-backed tool choice, Live2D renderer behavior, long-duration crash/restart soak, and guardrails beyond the deterministic Memory/Audio/approval/runtime contracts.
