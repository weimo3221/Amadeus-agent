...
- Add richer planning quality evals, child-agent runners, richer durable subprocess resume/restart policy, and truly isolated child-runtime human approval checkpoints/workspace sandboxing on top of the new task graph store, WorkerContext builder, internal orchestrator service, optional process runner, single-task worker entrypoint, subprocess launcher, first-pass `WorkerRuntimeScope`, and stage-oriented attempt checkpoints.
