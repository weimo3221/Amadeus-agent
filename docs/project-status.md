...
- Added first tool-output continuation slice: worker `tool.finished` previews are saved as normalized task artifacts with tool metadata, and current-task artifacts are included in WorkerContext so retry/resume prompts can reuse previous tool output instead of rediscovering it.
...
- Add richer planning quality evals, child-agent runners, real file-state/idempotent tool continuation from saved phases, and truly isolated child-runtime workspace sandboxing on top of the new task graph store, WorkerContext builder, internal orchestrator service, optional process runner, single-task worker entrypoint, subprocess launcher, first-pass `WorkerRuntimeScope`, checkpoint-aware retry handoff, first-pass approval checkpoints, saved-phase resume prompts, and worker tool-output artifacts.
