...
- Expanded `scripts/eval_runtime_contracts.py` into a ten-scenario deterministic suite with structured JSON output: role/task context, task lifecycle, cross-runner contract matrix, isolated child-agent/session/summary handoff, orchestrator repair/dependency/artifact handoff, MCP execution, subprocess duplicate/cancel/non-zero-exit/periodic-recovery faults, workspace-copy/symlink/path/write sandbox boundaries, action-specific approval target/expiry scope, and durable approval/override audit metadata. `npm test` runs this suite through `npm run test:contracts`. The matrix also exposed that the full threaded runtime can crash after raw `fork` on macOS, so `ProcessTaskRunner` is explicitly disabled there in favor of `SubprocessTaskRunner`.
...
- Add broader eval coverage for tool choice, permissions, memory, Live2D, audio, and guardrails.
...
- The durable supervisor is a single-host SQLite/PID control plane. Single-primary exclusion, process adoption, cancellation, resource limits, and clean shutdown are covered, but longer supervisor+worker double-crash/restart soak and resource-usage telemetry remain future eval work.
