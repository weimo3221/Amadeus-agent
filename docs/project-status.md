...
- Latest long-task isolation/supervisor validation also passes: `python -m py_compile packages/amadeus/server.py packages/amadeus/workers.py`, `python -m unittest tests.test_task_worker tests.test_python_runtime_http`, full `npm test`, full `npm run typecheck`, and `python scripts/eval_runtime_contracts.py`. A supervised no-desktop stack on temporary ports `8890` / `8888` reported `runnerKind=subprocess`, `workspaceIsolation=copy`, and `supervisorRunning=true`, then released both ports after shutdown.
...
- Add independently deployable scheduler/supervisor recovery beyond the current runtime-owned periodic lease reclaim.

### Phase 12: Advanced Agent Features

Started: restricted `delegate_task` now runs a real tracked isolated child model loop through TaskWorker, and the Hermes-style prompt surface split is in place. Multi-child autonomous orchestration quality, richer graph flow, and durable multi-process restart policy are not done.
