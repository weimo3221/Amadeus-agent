...
- Add richer planning quality evals, child-agent runners, override audit/policy tuning, and truly isolated OS/process-level child-runtime workspace sandboxing on top of the new task graph store, WorkerContext builder, internal orchestrator service, optional process runner, single-task worker entrypoint, subprocess launcher, first-pass `WorkerRuntimeScope`, checkpoint-aware retry handoff, first-pass approval checkpoints, saved-phase resume prompts, first-pass worker sandbox modes, and worker tool-output/file-state artifacts.
- Refine approval checkpoints with richer policy coverage and tuning for destructive/sensitive/low-confidence actions beyond the first classifier set.
