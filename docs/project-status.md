...
- Added deterministic runtime contract eval script at `scripts/eval_runtime_contracts.py` covering role identity, active/recent task context, task lifecycle, orchestrator graph repair/dispatch/synthesis, MCP tool schema/execution contracts, worker workspace-copy isolation, execution-tool workspace sandbox guards, and high-risk worker approval blocking.
