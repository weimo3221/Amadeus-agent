...
- Expanded `scripts/eval_runtime_contracts.py` into a nine-scenario deterministic suite with structured JSON output: role/task context, task lifecycle, cross-runner contract matrix, isolated child-agent/session/summary handoff, orchestrator repair/dependency/artifact handoff, MCP execution, subprocess duplicate/cancel/non-zero-exit/periodic-recovery faults, workspace-copy/symlink/path/write sandbox boundaries, and action-specific approval expiry/scope. `npm test` runs this suite through `npm run test:contracts`. The matrix also exposed that the full threaded runtime can crash after raw `fork` on macOS, so `ProcessTaskRunner` is explicitly disabled there in favor of `SubprocessTaskRunner`.
...
- Add richer planning quality evals, multi-child graph orchestration, override audit/policy tuning, and stronger OS-native sandbox backends on top of the tracked isolated child runner, task graph store, WorkerContext builder, internal orchestrator service, subprocess launcher, copy-isolated runtime, first-pass `WorkerRuntimeScope`, checkpoint-aware retry handoff, approval checkpoints, resume prompts, sandbox modes, and worker tool-output/file-state artifacts.
- Continue refining approval checkpoints with richer policy coverage, UI copy, and tuning for destructive/sensitive/low-confidence actions beyond the current classifier set.
