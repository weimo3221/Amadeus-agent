...
- Latest long-task isolation validation also passes: `python -m py_compile packages/amadeus/agent.py packages/amadeus/server.py packages/amadeus/workers.py packages/amadeus/worker_policy.py packages/amadeus/task_worker_entrypoint.py packages/amadeus/tools/terminal.py packages/amadeus/tools/execute_code.py packages/amadeus/tools/workspace_sandbox.py scripts/eval_runtime_contracts.py`, `python -m unittest tests.test_task_worker tests.test_tool_runtime tests.test_python_agent_runtime`, and `python scripts/eval_runtime_contracts.py`.
...
- Added first process execution slices: `TaskWorker` now runs behind a `TaskRunner` contract, the default remains the in-process thread-pool runner, `AMADEUS_TASK_RUNNER=process` can select an optional POSIX fork-backed runner, `amadeus.task_worker_entrypoint` can run one task in a dedicated process from `AMADEUS_TASK_ID` plus a SQLite database path, and `AMADEUS_TASK_RUNNER=subprocess` can launch that entrypoint as an external subprocess with bounded concurrency, `AMADEUS_TASK_RUN_ID`, `AMADEUS_WORKER_PROFILE`, parent-side wait/terminate handling, and first-pass non-zero exit reclaim back into the task retry/failure state machine. Subprocess-loss attempts are marked `abandoned` so they stay distinct from model/runtime failed attempts. Worker `workerProfile`, `allowedToolsets`, `disallowedTools`, task workspace hints, and task sandbox mode now narrow the child turn's available tool schemas, prompt hints, execution checks, ToolContext metadata, audit metadata, and workspace root resolution through an `AgentRuntime` `WorkerRuntimeScope`; workspace hints outside the current session workspace fail before model execution with a `worker_scope_invalid` attempt checkpoint, while `read_only`, `workspace_write`, and `workspace_execute` sandbox modes hide or block mutation/execution tools before handler execution with `worker_sandbox_denied` audit records. Worker ask-tool policy is profile-aware: approved tools such as researcher `web_extract` and coder `patch` can run with `worker_auto_approved`, while other ask tools block the task with a tool-specific approval checkpoint instead of opening interactive prompts or failing outright. Attempt checkpoints now record phases such as context build, scope validation, model turn start, assistant output, tool completion, error, cancellation, review blocking, worker tool approval blocking, and completion, with heartbeat preserving the latest phase for restart diagnostics. Worker tool result previews are stored as task artifacts and rehydrated into later WorkerContext prompts as `<task-artifacts>`, giving resumed workers concrete tool-output handoff before they decide whether to repeat work. Patch/write/read/search/terminal tool previews now add first-pass resume metadata such as affected files, observed files, commands, exit codes, changed flags, idempotency hints, and workspace file manifests with size/mtime/SHA-256 where available. `WorkerContext` re-verifies saved file manifests against current workspace state and exposes unchanged/changed/unverifiable results plus file resume policies such as `skip_redundant_mutation` and `reinspect_before_mutation` before the worker decides whether to repeat work. Stale-lease recovery and subprocess-loss retry now promote the latest running attempt checkpoint into task-level `resumeFrom` checkpoint and handoff summary for the next worker turn, and WorkerContext turns that into phase-specific resume instructions plus approval-specific saved-phase guidance.
