...
- Add richer planning quality evals, child-agent runners, richer durable subprocess restart/checkpoint policy, and truly isolated child-runtime permission prompts/workspace sandboxing on top of the new task graph store, WorkerContext builder, internal orchestrator service, optional process runner, single-task worker entrypoint, subprocess launcher, and first-pass `WorkerRuntimeScope`.
