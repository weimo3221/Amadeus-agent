...
- Python task worker now honors `reviewRequired`: successful worker output enters `blocked` with a review reason, task-level `approval_required` checkpoint, and handoff summary instead of `succeeded`; `POST /tasks/{id}/approve` finalizes it with an `approved` checkpoint. Worker ask-tools that are not profile-auto-approved now block the task with an approval checkpoint instead of failing the worker outright; `POST /tasks/{id}/resume` records the approved tool in the checkpoint so the next worker run can auto-approve that tool once through `WorkerRuntimeScope`.
...
- Added first human approval checkpoint slice: review-required worker output now persists a task-level `approval_required` checkpoint plus handoff summary while blocked, approval writes an `approved` checkpoint, and worker ask-tools outside the profile auto-approve set now block with a tool-specific approval checkpoint. Manual resume records either `approval_resume_requested` or `blocked_resume_requested`; when a tool was approved, the resumed `WorkerRuntimeScope` carries that ask-tool as a one-run approval and WorkerContext emits a constrained resume strategy that says to use the approved tool only for the blocked step.
- Added first tool-output/file-state continuation slice: worker `tool.finished` previews are saved as normalized task artifacts with tool metadata, current-task artifacts are included in WorkerContext, and patch/write/read/search/terminal artifacts expose resumeKind, affected/observed files, command state, idempotency hints, file manifests with size/mtime/SHA-256, current manifest verification, and `fileResumePolicy` instructions so retry/resume prompts can skip redundant mutations or reinspect changed files before repeating work. Main UI task details now render those policies as auditable artifact card callouts with status and instructions.
...
- Add richer planning quality evals, child-agent runners, runtime-level enforcement for verified manifest policies, and truly isolated child-runtime workspace sandboxing on top of the new task graph store, WorkerContext builder, internal orchestrator service, optional process runner, single-task worker entrypoint, subprocess launcher, first-pass `WorkerRuntimeScope`, checkpoint-aware retry handoff, first-pass approval checkpoints, saved-phase resume prompts, and worker tool-output/file-state artifacts.
- Refine approval checkpoints with more granular destructive/sensitive/low-confidence policies, clearer UI approval copy, and per-action expiry.
