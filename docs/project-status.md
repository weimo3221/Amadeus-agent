...
- Python task worker now honors `reviewRequired`: successful worker output enters `blocked` with a review reason, task-level `approval_required` checkpoint, and handoff summary instead of `succeeded`; `POST /tasks/{id}/approve` finalizes it with an `approved` checkpoint. `POST /tasks/{id}/resume` returns blocked tasks to `queued` and records whether the resume came from an approval checkpoint or a generic blocked state.
...
- Added first human approval checkpoint slice: review-required worker output now persists a task-level `approval_required` checkpoint plus handoff summary while blocked, approval writes an `approved` checkpoint, and manual resume from blocked state writes either `approval_resume_requested` or `blocked_resume_requested` for restart/debug visibility.
...
- Add richer planning quality evals, child-agent runners, tool/state-level continuation from saved phases, and truly isolated child-runtime approval/workspace sandboxing on top of the new task graph store, WorkerContext builder, internal orchestrator service, optional process runner, single-task worker entrypoint, subprocess launcher, first-pass `WorkerRuntimeScope`, checkpoint-aware retry handoff, and first-pass review approval checkpoints.
- Extend approval checkpoints beyond `reviewRequired` worker completion into destructive, sensitive, or low-confidence action policies.
