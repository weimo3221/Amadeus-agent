...
- Worker ask-tool approval is now action-specific when the blocked event provides action metadata: terminal exact command keys, process action keys, and path/target keys resume as one-run `approvedToolActions`, while old tool-wide approval remains only as a legacy fallback for checkpoints without an action key.
...
- Refine approval checkpoints with clearer UI approval copy, time-based expiry, more action classifiers, and policy coverage for destructive/sensitive/low-confidence actions beyond the first terminal/process/path action keys.
