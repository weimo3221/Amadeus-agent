...
Started: the first restricted `delegate_task` research/search tool, session task worker, and Hermes-style prompt surface split are in place. Full sub-agent orchestration and durable multi-process restart/reclaim policy are not done.
...
- Add richer planning quality evals, child-agent runners, and durable subprocess restart/reclaim handling on top of the new task graph store, WorkerContext builder, internal orchestrator service, optional process runner, single-task worker entrypoint, and subprocess launcher.
