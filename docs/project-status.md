...
- Python now owns the preferred model/tool/memory/behavior path for a turn:
  ...
  - assembles API-call-time context from the active runtime memory provider, recent messages, active plan/todo/task reference blocks, and diagnostics; the default `hybrid_runtime` provider preserves the original `builtin_runtime` summaries, query-filtered accepted memory items, current-session FTS retrieval, and memory tools, then can fill sparse retrieval slots from global SQLite FTS when no external provider is configured
...
- Phase 7 ToolRuntime first slice is in place.
  ...
  - `ContextAssembler` now consumes a runtime memory manager. Stable Markdown memory stays in the prompt layer; the default `hybrid_runtime` provider wraps and preserves the original `builtin_runtime` provider, exposing derived SQLite/session artifacts such as summaries, query-filtered accepted structured memory, sanitized FTS snippets, and default memory tools when no external provider is configured. `configs/runtime.yaml` can switch back to `builtin_runtime`, or keep `hybrid_runtime` while disabling `memory.globalRetrievalFallback`. If an external provider is configured, it replaces the runtime provider surface for prefetch and memory tools. Active plans, todos, task state, recent task results, and memory provider snippets are attached to the current user message as non-persistent reference context. `memory.context.used` diagnostics are retained in a per-session in-memory ring buffer and include retrieval provider metadata.
...
- `memory_provider.py` now has a runtime memory provider registry. The original `builtin_runtime` provider is preserved, while the default `hybrid_runtime` provider wraps it and adds an optional global FTS fallback lane for sparse current-session retrieval. A configured external provider still replaces that prefetch/tool surface. Raw transcripts remain separate log data.
...
- Added the first local embedding resource boundary for Memory v2: Python exposes `GET /memory/embedding/config`, `POST /memory/embedding/deploy`, and `POST /memory/embedding/cancel` for BGE-M3 (`BAAI/bge-m3`) local deployment via FlagEmbedding. The Main UI Config Center now has a Memory tab showing configuration, optional dependency state, model cache state, deployment phase, and cancel/refresh controls. Runtime memory now has a `hybrid_runtime` provider boundary that preserves the original local provider and adds a global SQLite FTS fallback lane; BGE-M3 vector indexing/backfill remains the next retrieval step.
