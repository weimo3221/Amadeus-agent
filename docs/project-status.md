...
Started: the storage/API/UI foundation and session-scoped task worker are in place, with worker retry, periodic stale-lease recovery, the first task-graph persistence layer, dependency-aware runnable selection, isolated WorkerContext prompts, the first internal orchestrator skeleton, first-pass model-backed graph decomposition/repair/lifecycle events/root synthesis, profile/toolset policy validation, an optional POSIX process runner, a single-task worker process entrypoint, and a production-default independently supervised subprocess/copy-isolated runner with process adoption, resource limits, process-group cancellation, lifecycle audit events, non-zero exit reclaim, and abandoned-attempt marking. Autonomous scheduling, richer graph UI semantics, deeper repair/eval loops, multi-child orchestration quality, and long double-crash/restart soak coverage are not done.
...
Started: restricted `delegate_task` now runs a real tracked isolated child model loop through TaskWorker, and the Hermes-style prompt surface split is in place. Multi-child autonomous orchestration quality, richer graph flow, and long durable restart/soak validation are not done.
...
- Add richer planning quality evals, multi-child graph orchestration, broader approval/override policy tuning, and wider native-sandbox deployment coverage on top of the tracked isolated child runner, task graph store, WorkerContext builder, internal orchestrator service, subprocess launcher, copy-isolated runtime, bubblewrap/Seatbelt boundary, first-pass `WorkerRuntimeScope`, checkpoint-aware retry handoff, approval checkpoints, resume prompts, sandbox modes, worker tool-output/file-state artifacts, and durable approval/override audit trail.
