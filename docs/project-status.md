...
- Add richer planning quality evals, child-agent runners, richer durable subprocess restart policy, and deeper worker-profile permission/workspace policy on top of the new task graph store, WorkerContext builder, internal orchestrator service, optional process runner, single-task worker entrypoint, and subprocess launcher.
