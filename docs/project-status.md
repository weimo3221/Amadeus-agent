...
- Added first human approval checkpoint slice: review-required worker output now persists a task-level `approval_required` checkpoint plus handoff summary while blocked, approval writes an `approved` checkpoint, and worker ask-tools outside the profile auto-approve set now block with a tool-specific approval checkpoint. Manual resume records either `approval_resume_requested` or `blocked_resume_requested`; when a tool was approved, the resumed `WorkerRuntimeScope` carries that ask-tool as a one-run approval.
...
- Add richer planning quality evals, child-agent runners, tool/state-level continuation from saved phases, and truly isolated child-runtime workspace sandboxing on top of the new task graph store, WorkerContext builder, internal orchestrator service, optional process runner, single-task worker entrypoint, subprocess launcher, first-pass `WorkerRuntimeScope`, checkpoint-aware retry handoff, and first-pass approval checkpoints.
