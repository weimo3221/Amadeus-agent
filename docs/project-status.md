...
- Expanded `scripts/eval_runtime_contracts.py` into an eight-scenario deterministic suite with structured JSON output: role/task context, task lifecycle, cross-runner contract matrix, orchestrator repair/dependency/artifact handoff, MCP execution, subprocess duplicate/cancel/non-zero-exit/periodic-recovery faults, workspace-copy/symlink/path/write sandbox boundaries, and action-specific approval expiry/scope. `npm test` now runs this suite through `npm run test:contracts`. The matrix also exposed that the full threaded runtime can crash after raw `fork` on macOS, so `ProcessTaskRunner` is explicitly disabled there in favor of `SubprocessTaskRunner`.
...
- Add richer planning quality evals, child-agent runners, override audit/policy tuning, and stronger OS-native sandbox backends on top of the new task graph store, WorkerContext builder, internal orchestrator service, optional process runner, single-task worker entrypoint, subprocess launcher, copy-isolated child runtime, first-pass `WorkerRuntimeScope`, checkpoint-aware retry handoff, first-pass approval checkpoints, saved-phase resume prompts, first-pass worker sandbox modes, and worker tool-output/file-state artifacts.
