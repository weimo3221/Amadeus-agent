...
- Added first human approval checkpoint slice: review-required worker output now persists a task-level `approval_required` checkpoint plus handoff summary while blocked, approval writes an `approved` checkpoint, and worker ask-tools outside the profile auto-approve set now block with a tool/action-specific approval checkpoint. Manual resume records either `approval_resume_requested` or `blocked_resume_requested`; when a tool action was approved, the resumed `WorkerRuntimeScope` carries that action key as a one-run approval and WorkerContext emits a constrained resume strategy that says to use only that approved command/action/path/target for the blocked step. Main UI task details render the approval action key/label, risk level/labels, tool name, and expiry countdown/status so users do not need to inspect raw checkpoint JSON.
