...
- Add richer planning quality evals, multi-child graph orchestration, broader approval/override policy tuning, and stronger OS-native sandbox backends on top of the tracked isolated child runner, task graph store, WorkerContext builder, internal orchestrator service, subprocess launcher, copy-isolated runtime, first-pass `WorkerRuntimeScope`, checkpoint-aware retry handoff, approval checkpoints, resume prompts, sandbox modes, worker tool-output/file-state artifacts, and durable approval/override audit trail.
