...
- `subprocess_started.metadata.osSandbox`, the durable process registry, and `/runtime/health` report the requested OS sandbox mode, selected backend, executable, enforcement flag, and fallback reason. Consumers must treat `enforced=false` as copy/env/tool guarding only, not kernel containment.
- `POST /tasks/{id}/cancel` marks active tasks as `cancelled`, records one `cancelled` task event, and asks the backing agent turn to cancel when one is running. Cancelling an orchestrator root first cascades to all active graph children and records `graph.cancelled`; cancelling a child directly remains local to that child.
- Applying a graph moves the root to `blocked` with checkpoint phase `orchestrator_waiting`, and `maxConcurrency` is enforced before and during child claim. `POST /tasks/{id}/replan` can append one policy-preserving replacement for a failed/cancelled child within the root `maxReplans` budget, atomically rewire pending dependencies, and record `graph.replanned`.
