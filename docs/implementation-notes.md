...
`npm test` now runs Python `unittest` coverage for the Python runtime path. Keep these tests focused on deterministic behavior that does not require a live model provider. Current gaps are HTTP endpoint tests, bridge relay tests, and desktop/WebSocket integration coverage.
