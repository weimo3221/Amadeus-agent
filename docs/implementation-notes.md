...
- Memory review candidates are stored separately from durable memory. They are exposed through `GET/POST /memory/review/candidates`, accepted through `POST /memory/review/accept`, and rejected through `POST /memory/review/reject`; only acceptance writes to `memory_items`.
