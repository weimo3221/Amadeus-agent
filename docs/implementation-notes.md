...
Task worker execution is split at the `TaskRunner` boundary. `TaskWorker` owns task claim/retry/recovery/cancel/event semantics, while `InProcessTaskRunner` owns the default thread-pool submission mechanism. `SynchronousTaskRunner` exists for single-task entrypoints and deterministic tests. `ProcessTaskRunner` is available as an optional POSIX fork-backed runner behind the same contract and can be selected with `AMADEUS_TASK_RUNNER=process`; `amadeus.task_worker_entrypoint` can run one task in a dedicated process from `--task-id` / `AMADEUS_TASK_ID` plus `--database` / `AMADEUS_MEMORY_DB`; and `SubprocessTaskRunner` can launch that entrypoint with `AMADEUS_TASK_RUNNER=subprocess`, bounded concurrency, `AMADEUS_TASK_RUN_ID`, `AMADEUS_WORKSPACE`, `AMADEUS_WORKER_PROFILE`, parent-side process join/terminate handling, first-pass non-zero exit reclaim, and `abandoned` attempt marking for subprocess-loss cases. Worker turns now apply `workerProfile` / `allowedToolsets` / `disallowedTools` / task workspace hints through a temporary `AgentRuntime` `WorkerRuntimeScope`, so model tool schemas, prompt hints, execution checks, tool context metadata, audit metadata, and workspace root resolution are narrowed for the child task in addition to normal role scope. Task workspace hints must resolve inside the current session workspace; invalid worker workspaces fail the task before model execution with a `worker_scope_invalid` attempt checkpoint instead of silently escaping the role workspace or retrying a deterministic configuration error. Worker permission policy is profile-aware: ask-tools are not allowed to open interactive prompts from background worker turns; narrow profile-approved ask tools such as researcher `web_extract` and coder `patch` are auto-approved with `permissionDecision="worker_auto_approved"`. Other worker ask-tools now block the task with a tool-specific `approval_required` checkpoint; after the user resumes, the checkpoint grants that ask-tool to the next `WorkerRuntimeScope` as a one-run worker auto-approval. Worker attempts now persist stage-oriented checkpoints during context build, scope validation, model-turn start, tool completion, assistant output, error, cancellation, review blocking, worker tool approval blocking, and completion; heartbeats preserve the latest structured checkpoint instead of replacing it with a bare running marker, and previous attempt prompts include checkpoint phase/last-event hints. Worker `tool.finished` previews are saved as normalized task artifacts with tool metadata, and current-task artifacts are included in WorkerContext as `<task-artifacts>` so resumed workers can reuse prior tool output. Successful tool events now include a compact JSON result preview when no policy preview exists, letting worker artifacts capture file-state/idempotency metadata for patch/write/read/search/terminal tools: affected files, observed files, commands, exit codes, changed flags, explicit hints to verify saved state before repeating work, workspace file manifests with size, mtime, and SHA-256 where files are available and bounded, current manifest verification during `WorkerContext` build, and first-pass `fileResumePolicy` instructions such as `skip_redundant_mutation`, `reinspect_before_mutation`, and `reuse_observation`. Main UI task artifact cards render those verified file resume policies with status tags, paths, and instructions so resume behavior is auditable without reading raw JSON. The same verified policies now enter `ToolLoopGuardrail` through `WorkerRuntimeScope`, blocking unchanged same-tool same-path repeat mutations before handler execution and requiring a same-turn `read_file` refresh before mutating a path whose saved mutation artifact is changed. Stale-lease recovery and subprocess-loss retry now promote the last running attempt checkpoint into the task checkpoint as `resumeFrom` plus a handoff summary, so the next worker turn sees the interrupted phase rather than only a generic retry error. Review-required completions now persist a task-level `approval_required` checkpoint plus handoff summary while blocked; approval writes `approved`, and manual resume records either `approval_resume_requested` or `blocked_resume_requested`. `WorkerContext` now turns `resumeFrom` into an explicit `<resume-strategy>` prompt section, guiding the next worker to verify partial assistant output, avoid duplicate work, or change approach after errors depending on the saved phase. For approval resumes, the strategy also lists approved ask-tools and tells the worker to use them only for the blocked step, not as broad or permanent permission. This is still not a full durable process supervisor or isolated child runtime: more granular destructive/sensitive action approval policy, finer file-resume override semantics, and workspace sandboxing need to be added above the launcher. Running tasks now persist a lease (`leaseOwner`, `leaseExpiresAt`, `runnerKind`) in addition to the legacy `claimLock`; worker heartbeat threads renew the lease during long turns, startup recovery requeues expired leases or legacy stale heartbeats, and terminal transitions clear lease state. Current worker turns are built from an isolated `WorkerContext` containing the task spec, acceptance criteria, context hints, task/dependency artifacts, and previous attempt history; do not fall back to parent conversation replay for child/worker execution. Do not introduce a parallel Kanban/swarm scheduler until this runner boundary has been exercised.
