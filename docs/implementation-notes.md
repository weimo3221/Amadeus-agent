...
- Runtime observability now has a first read-only aggregation layer: `GET /runtime/observability?sessionId=...` combines runtime health, recent task/task-graph state, role-scoped skill inventory, effective tool visibility, ToolRuntime/MCP audit failures, and recent `memory.context.used` diagnostics without triggering provider calls or MCP discovery. The Vue Main UI has a Run Observability page backed by that endpoint; it is the cross-module snapshot layer above the existing deeper task, memory, skill, MCP, and config views.
- P3 release preflight now has an explicit first slice: `scripts/release_check.py` validates release/E2E/soak/updater wiring, `scripts/runtime_soak.py` polls an already-running Python runtime and Node bridge through `/runtime/health`, `/runtime/observability`, and `/health`, and the Electron main process has a packaged-only, env-gated `electron-updater` bootstrap. Auto-update checks stay disabled unless `AMADEUS_AUTO_UPDATE=1` and `AMADEUS_UPDATE_FEED_URL` are configured. This is release hardening infrastructure, not yet code signing, notarization, installer generation, or a production update feed.
