...
- `packages/amadeus/memory_provider.py` now defines the runtime memory provider layer. `LocalRuntimeMemoryProvider` is always available and exposes derived SQLite/session memory artifacts such as summaries, accepted structured facts, and FTS snippets; optional external providers contribute bounded `<external-memory-context>` snippets. Raw session transcripts are not provider outputs and remain accessible only through explicit transcript APIs/tools.
...
- Memory v2 should be treated as core-implemented and entering consolidation mode. `ContextAssembler` now consumes a built-in runtime memory provider for API-call-time summary, query-filtered structured memory, and FTS retrieval injection, plus optional external providers and `memory.context.used` diagnostics; the runtime retains the most recent diagnostics per session in an in-memory ring buffer for developer inspection. The next memory work should focus on summary/profile policy, review quality, external provider quality, overflow behavior, and diagnostics endpoints rather than basic storage primitives.
