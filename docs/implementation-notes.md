...
Long-task decomposition should go through the internal `OrchestratorService`, not direct model-authored database mutations. The current service accepts structured graph payloads, validates task ids/dependencies/cycles, persists child tasks and edges through the normal task store, and dispatches only dependency-ready children. Model-backed `specify` / `decompose` should be added above this service with fixed JSON schemas and conservative fallback behavior.
