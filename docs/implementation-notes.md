...
- Main UI now exposes a first MCP management surface. The MCP tab edits `tools.mcp.enabled`, default permission, and HTTP JSON-RPC server entries, saves them through Python `POST /tools/config`, and rebuilds the Python `ToolRegistry` immediately so server diagnostics and model tool schemas refresh without a manual restart. This is still HTTP JSON-RPC only; stdio/SSE server lifecycle management remains future work.
