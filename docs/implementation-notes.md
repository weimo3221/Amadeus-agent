...
- Lipsync is still a priority experience gap, but the desktop now does a first real upgrade: runtime audio playback drives `ParamMouthOpenY` from Web Audio amplitude analysis, while the older timed mouth loop remains as fallback for speech synthesis and environments where media-element analysis is unavailable. Future work should move from amplitude-only toward phoneme-aware cues carried through the runtime/harness boundary.
...
- The desktop side now prefers audio-driven lipsync for runtime audio by sampling the playing `HTMLAudioElement` through Web Audio `AnalyserNode` and mapping waveform energy to `ParamMouthOpenY`. The next large architectural gap is completing the runtime/harness layer: audio harness decisions, richer Live2D commands, phoneme-aware cues, and later skills/tasks.
