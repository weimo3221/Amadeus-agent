...
Task worker execution is split at the `TaskRunner` boundary. `TaskWorker` owns task claim/retry/recovery/cancel/event semantics, and production server startup now defaults to `AMADEUS_TASK_RUNNER=subprocess` plus `AMADEUS_TASK_WORKSPACE_ISOLATION=copy`; `in_process`, synchronous, and supported non-macOS POSIX fork runners remain explicit compatibility/test choices. The fork runner is rejected on macOS because the full threaded Python runtime is not safe to execute after `fork`; use the external subprocess runner there. `amadeus.task_worker_entrypoint` runs one task in a dedicated process from `--task-id` / `AMADEUS_TASK_ID` plus `--database` / `AMADEUS_MEMORY_DB`. `SubprocessTaskRunner` enforces bounded non-blocking submission, suppresses duplicate active task launches, starts each child in its own process group, supports task-specific termination, records `subprocess_started` / `subprocess_termination_requested` / `subprocess_exited` events, reclaims non-zero exits, and marks subprocess-loss attempts `abandoned`. The runtime-owned recovery supervisor reruns expired-lease recovery and queued-task dispatch at `AMADEUS_TASK_RECOVERY_INTERVAL_SECONDS` (default 15 seconds), so a child that outlives a runtime restart can finish normally while a later orphan failure is reclaimed after its lease expires. The Python server reports this state under `/runtime/health` and shuts the scheduler/worker down on SIGTERM. Copy isolation creates a per-run workspace under `AMADEUS_TASK_WORKSPACE_SANDBOX_ROOT` or the memory database directory, skips generated/heavy directories and external symlinks, passes `AMADEUS_WORKER_WORKSPACE_OVERRIDE`, `AMADEUS_WORKER_WORKSPACE_ISOLATION`, and `AMADEUS_WORKER_WORKSPACE_SOURCE` to the child, and runs the entrypoint with that copied workspace as cwd.
