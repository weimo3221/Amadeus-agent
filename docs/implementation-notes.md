...
Worker turns now apply `workerProfile` / `allowedToolsets` / `disallowedTools` / task workspace hints through a temporary `AgentRuntime` `WorkerRuntimeScope`, so model tool schemas, prompt hints, execution checks, tool context metadata, audit metadata, and workspace root resolution are narrowed for the child task in addition to normal role scope. Task workspace hints must resolve inside the current session workspace; invalid worker workspaces fail the task before model execution with a `worker_scope_invalid` attempt checkpoint instead of silently escaping the role workspace or retrying a deterministic configuration error. Worker permission policy is profile-aware: ask-tools are not allowed to open interactive prompts from background worker turns; narrow profile-approved ask tools such as researcher `web_extract` and coder `patch` are auto-approved with `permissionDecision="worker_auto_approved"`. Other worker ask-tools now block the task with a tool/action-specific `approval_required` checkpoint containing an action key, label, risk level, and risk labels when available; after the user resumes, the checkpoint grants that specific action key to the next `WorkerRuntimeScope` as a one-run worker auto-approval with a default 15-minute expiry. Legacy tool-wide approval remains only for older checkpoints that lack an action key.
