...
- `amadeus.memory_safety`: pre-persistence safety checks for memory review candidates, currently blocking secret-like content, temporary debug/run state, uncertain claims, and overly specific local/cache/generated paths before anything reaches the human review queue.
