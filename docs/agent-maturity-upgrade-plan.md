...
- ToolRuntime 已有 registry、permission metadata、structured result、审计记录、超时、cooperative cancellation、结果压缩、重复失败 guardrail 和 first-pass no-progress detection，但还缺持久化审计和更强语义级 no-progress detection。
...
状态：进行中。已完成 Python `ToolRegistry`、`configs/tools.yaml` loading、`ToolContext` / `ToolResult` 第一版、duration/failure code、first-pass timeout handling、cooperative cancellation、result preview/compression、permission-aware schema projection、first-pass audit records、repeated-failure guardrail、first-pass no-progress detection。剩余重点是持久化审计、per-tool result policies 和更强语义级 no-progress detection。
