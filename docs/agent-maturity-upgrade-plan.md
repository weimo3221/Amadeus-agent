...
- task/job 体系已具备持久化状态、attempt/lease/heartbeat、dependency graph、artifact handoff、subprocess + copy isolation、周期恢复和结构化健康状态；真正受限的 isolated child model loop、独立 durable supervisor 和更完整 graph flow 仍未收口。
- runtime contract eval 已通过 `npm test` 覆盖八个确定性场景，并保留 typecheck/unit/bridge/renderer/Electron E2E；仍缺模型质量、长时间 crash/restart soak、Memory 与 Live2D/audio interaction 评估。
...
- isolated child model loop：下一阶段复用 `WorkerContext`、`TaskWorker` 和 `WorkerRuntimeScope`，替换现有启发式 `delegate_task`，不再引入另一套任务状态机。
...
- Worker ask-tool approval 已从纯工具级别推进到第一版 action-specific policy：terminal 命令会生成 exact command action key，process `list/status/kill` 会按 action 区分，文件/网页类工具会按 path/target 生成 key；blocked checkpoint 会保存 action label/risk metadata，第一批 classifier 会标记 destructive/privileged shell、installer、network script/access、sensitive data/path、workspace-external path、whole-file write、bulk replace、insecure/sensitive URL 和 unknown target；resume 后只把该 action key 放入下一次 `WorkerRuntimeScope`，并带来自 `tasks.workerApprovalActionTtlSeconds` / `AMADEUS_WORKER_APPROVAL_ACTION_TTL_SECONDS` 的过期时间，默认 15 分钟；旧的 tool-wide auto approval 只兼容没有 action key 的历史 checkpoint。
...
- 仍需补更细的危险/敏感/低置信度动作 approval policy 调优、file-resume override audit/policy 调优、OS/process-level workspace sandbox，以及真正 isolated child runtime。
...
- Status: production server now defaults to external `SubprocessTaskRunner` with workspace-copy isolation. The optional POSIX fork-backed `ProcessTaskRunner` remains behind the same contract for supported non-macOS systems and can be selected with `AMADEUS_TASK_RUNNER=process`.
...
- 已实现 verified manifest policy 的第一版 runtime-level enforcement、file-resume override 执行语义、用户可操作的 file-resume override 流程、action-specific approval key 的可配置过期、第一批 destructive/sensitive action classifier、第一版 worker sandbox mode enforcement，以及 task-detail approval action/risk/expiry/inline resume copy 展示；仍需补更多 approval policy 调优、file-resume override audit/policy 调优和 OS/process-level 工作区沙箱策略。
...
- Runtime-owned periodic supervisor now performs expired-lease recovery and queued redispatch and exposes diagnostics through `/runtime/health`; an independently deployed durable supervisor with process adoption and resource limits remains future work.
