...
- 仍需实现文件状态/幂等工具级真正续跑的进程 resume/restart 策略，以及更细的 approval checkpoint policy 和工作区沙箱策略。
