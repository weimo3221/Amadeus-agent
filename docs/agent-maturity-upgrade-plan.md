...
- Live2D/audio 已有第一版 harness/runtime 边界：Live2D harness 映射 `assistant.state` 到 `character.behavior`，本地模型库支持切换，Python audio 支持 GPT-SoVITS 配置和 macOS `say` fallback。缺口是 audio harness、capability feedback、playback feedback、丰富命令和音频驱动 lipsync。
