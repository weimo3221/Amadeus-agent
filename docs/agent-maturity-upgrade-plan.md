...
- 仍需补更细的危险/敏感/低置信度动作 approval policy 调优、file-resume override audit/policy 调优、workspace sandbox policy，以及真正 isolated child runtime。
...
- 已实现 verified manifest policy 的第一版 runtime-level enforcement、file-resume override 执行语义、用户可操作的 file-resume override 流程、action-specific approval key 的可配置过期、第一批 destructive/sensitive action classifier，以及 task-detail approval action/risk/expiry/inline resume copy 展示；仍需补更多 approval policy 调优、file-resume override audit/policy 调优和工作区沙箱策略。
