...
- task/job 体系已具备持久化状态、attempt/lease/heartbeat、dependency graph、artifact handoff、subprocess + copy isolation、独立 SQLite lease supervisor、durable process registry、进程接管、资源限制、Linux bubblewrap/macOS Seatbelt backend 边界、结构化健康状态，以及第一版受限 isolated child model loop；多 child 自主编排和更完整 graph flow 仍未收口。
- runtime contract eval 已通过 `npm test` 覆盖十四个确定性场景并输出 per-check metrics，包括 bounded graph replan、durable supervisor restart/adoption/lost-process soak、Memory recall/safety、Audio fallback/lipsync；仍缺模型质量、长时间 supervisor/worker crash-restart soak 与真实 Live2D renderer interaction 评估。
...
- 仍需补更广的危险/敏感/低置信度动作 approval/override policy 调优、native sandbox 部署矩阵、多 child 编排质量和异步 handoff UX。
...
- 已实现 verified manifest policy 的第一版 runtime-level enforcement、file-resume override 执行语义、用户可操作的 file-resume override 流程与 durable audit event、target-specific approval key 的可配置过期、第一批 destructive/sensitive action classifier、approval block/resume/review 与成功工具执行的关联审计、worker sandbox mode enforcement、bubblewrap/Seatbelt backend boundary，以及 task-detail approval action/risk/expiry/inline resume copy 展示；仍需补更广的 approval/override policy 调优和 native backend 部署覆盖。
...
- OS-native backend 第一版已落地：`auto` 探测 Linux bubblewrap/macOS Seatbelt，`required` fail closed，bubblewrap 用只读 host + 当前 workspace/state mount，Seatbelt 只开放当前 workspace 与 SQLite sidecar 写入；backend/enforcement/fallback reason 写入 lease/process/event/health。当前开发机 Seatbelt probe 被宿主拒绝，`auto` 明确降级，不会伪报 enforced。
- Python `/runtime/health` 会暴露 external supervisor lease/process registry 与 OS sandbox 状态，并在 supervisor 缺失时降级；`scripts/dev_stack.py` 先启动 supervisor，再启动 runtime/bridge。剩余工作是多 child 编排、native backend 部署矩阵、长时间双重崩溃 soak、资源使用指标，以及跨主机调度（当前不在 SQLite 单机控制面的范围内）。
...
- Long-task/runtime eval 已集中到 `scripts/eval_runtime_contracts.py`：覆盖 decomposition JSON validity、dependency dispatch、bounded graph replan、worker isolation、artifact handoff、cancel/retry/recover、review gate、supervisor restart/adoption/lost-process soak、Memory recall/safety、Audio fallback/lipsync，以及 approval/override audit。
- 每个 runner 仍需至少有一组相同 task graph contract test；新增 runner、graph prompt/schema、approval policy、Memory/Audio runtime 改动必须跑 `npm run test:contracts` 或完整 `npm test`。
- 后续补模型驱动 planning/tool-choice quality、小时级 double-crash soak 与真实 Live2D renderer interaction eval。
