...
- 仍需实现 verified manifest policy 的 runtime-level enforcement/UI 展示，以及更细的 approval checkpoint policy 和工作区沙箱策略。
