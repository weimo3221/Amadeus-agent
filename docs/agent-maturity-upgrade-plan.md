...
- 已完成第一版 worker runtime scope：worker task 的 `workerProfile`、`allowedToolsets`、`disallowedTools` 和任务 workspace hints 会被解析成临时 `AgentRuntime` `WorkerRuntimeScope`，限制 child turn 的 tool schema、prompt hints、执行检查、ToolContext 元数据、audit metadata 和 workspace root resolution。
- 仍需补更细的 permission prompt policy、workspace sandbox policy、checkpoint/restart policy，以及真正 isolated child runtime。
...
- Worker profile/toolset/workspace policy now limits child tool visibility, execution, context metadata, audit metadata, and workspace root resolution through `AgentRuntime.worker_runtime_scope(...)`.
