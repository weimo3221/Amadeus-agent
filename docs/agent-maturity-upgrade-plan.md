...
- 仍需实现基于 saved manifest 的自动 verify/skip 进程 resume/restart 策略，以及更细的 approval checkpoint policy 和工作区沙箱策略。
