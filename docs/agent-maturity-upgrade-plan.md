...
- `packages/amadeus`
  ...
  - Python ToolRuntime：registry/config loading、permission metadata、structured `ToolResult`、repeated-failure guardrail。
...
- ToolRuntime 已有 registry、permission metadata、structured result 和重复失败 guardrail，但还缺审计记录、超时、取消、结果压缩、更强 no-progress detection。
