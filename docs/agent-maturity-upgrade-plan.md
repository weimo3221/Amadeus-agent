...
- `packages/amadeus`
  ...
  - Python ToolRuntime：registry/config loading、permission metadata、structured `ToolResult`、result compression、repeated-failure guardrail。
...
- ToolRuntime 已有 registry、permission metadata、structured result、审计记录、超时、结果压缩和重复失败 guardrail，但还缺 stronger cancellation、持久化审计和更强 no-progress detection。
...
状态：进行中。已完成 Python `ToolRegistry`、`configs/tools.yaml` loading、`ToolContext` / `ToolResult` 第一版、duration/failure code、first-pass timeout handling、result preview/compression、permission-aware schema projection、first-pass audit records、repeated-failure guardrail。剩余重点是 stronger cancellation、持久化审计、per-tool result policies 和 no-progress detection。
