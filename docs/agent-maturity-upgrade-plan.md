...
- `packages/amadeus`
  ...
  - Python 工具：`get_current_time`、`roll_dice`、`search_files`、`read_file`；`local_file_search` 作为旧名兼容 alias 保留。
  - Python SQLite message store。
  - Python ToolRuntime：registry/config loading、permission metadata、structured `ToolResult`、timeout/cancellation、result compression、`search_files` / `read_file` per-tool result policies、repeated-failure/no-progress guardrail。
...
- ToolRuntime 已有 registry、permission metadata、structured result、SQLite 持久化审计记录、超时、cooperative cancellation、结果压缩、`search_files` / `read_file` per-tool result policies、重复失败 guardrail 和 first-pass no-progress detection，但还缺更强语义级 no-progress detection。
...
- `get_current_time`、`roll_dice`、`search_files`、`read_file` 仍可工作，`local_file_search` 作为旧名兼容 alias。
- 断开 Python runtime 时 server 明确报错。
- `apps/server` 不再直接调用 LLM provider。

### Phase 2：统一 ToolRuntime

状态：进行中。已完成 Python `ToolRegistry`、`configs/tools.yaml` loading、`ToolContext` / `ToolResult` 第一版、duration/failure code、first-pass timeout handling、cooperative cancellation、result preview/compression、`search_files` / `read_file` per-tool result policies、permission-aware schema projection、SQLite-backed audit records、repeated-failure guardrail、first-pass no-progress detection。剩余重点是更多高容量工具的 result policies 和更强语义级 no-progress detection。
