...
- Worker ask-tool approval 已从纯工具级别推进到第一版 action-specific policy：terminal 命令会生成 exact command action key，process `list/status/kill` 会按 action 区分，文件/网页类工具会按 path/target 生成 key；blocked checkpoint 会保存 action label/risk metadata，resume 后只把该 action key 放入下一次 `WorkerRuntimeScope`，并带默认 15 分钟过期时间；旧的 tool-wide auto approval 只兼容没有 action key 的历史 checkpoint。
- Main UI task detail 现在会把 approval action label/key、tool name、risk level/labels 和 expiry 状态直接展示在 checkpoint 面板里，不再要求用户读 raw checkpoint JSON 才能判断批准的动作边界。
- 仍需补更细的危险/敏感/低置信度动作 approval policy、workspace sandbox policy、更细的 file-resume override 语义，以及真正 isolated child runtime。
...
- 已实现 verified manifest policy 的第一版 runtime-level enforcement、action-specific approval key 的默认过期、以及 task-detail approval action/risk/expiry 展示；仍需补更清晰的 inline approve/resume 文案、可配置 expiry、更多 action classifier、file-resume override 语义和工作区沙箱策略。
