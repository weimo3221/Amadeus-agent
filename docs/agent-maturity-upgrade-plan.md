...
Amadeus 现在已经有一个能跑的桌面 MVP：Electron 透明窗口、远程 Live2D 模型、WebSocket 事件、OpenAI-compatible 模型调用、SQLite 原始消息、工具调用、工具权限 prompt、Python sidecar、音频 fallback 都已经打通。它的问题不是“没有功能”，而是核心 agent runtime 还没有成熟：主循环、模型调用、记忆装配、工具调度、权限、行为策略仍主要散在 `apps/server/src/index.ts`，Python 侧还只是接口雏形和少量工具。
...
- `apps/server`
  ...
  - 非流式工具决策 + 流式最终回答。
  - SQLite 原始消息存储。
  - 工具 registry、`allow/ask/deny`、工具权限 request/response。
  - 调用 Python `/tools/execute` 和 `/audio/speak`。
- `packages/amadeus`
  - Python `server.py` sidecar。
  - Python 工具：`get_current_time`、`roll_dice`、`local_file_search`。
  - Python SQLite message store。
  - Python audio/live2d dataclass 合同雏形。
  - TypeScript `events.ts` 与 `tools.ts` bridge。

### 成熟 agent 差距

- 主循环没有 Python-owned。`apps/server/src/index.ts` 仍直接持有 LLM loop、context assembly、tool loop、memory persistence、behavior dispatch。
- Python `agent.py` 只是 placeholder，没有多轮 tool loop、streaming event、context assembly、guardrail、cancellation。
- 记忆只有 raw message replay，没有 conversation summary、user profile、事实抽取、跨会话搜索、记忆写入策略。
- 工具系统还不是成熟 runtime registry。缺少统一 ToolSpec、ToolContext、权限策略、审计日志、超时、并发、取消、结果压缩、工具循环防护。
...
- 没有 eval 和 regression harness。现在有 typecheck/build/smoke test，但缺少针对 agent 行为的可重复评估。
...
- `get_current_time`、`roll_dice`、`local_file_search` 仍可工作。
...
- 新增 `ToolSpec`、`ToolContext`、`ToolResult`、`ToolPermissionPolicy`。
- 工具 schema、权限、display name、handler 全部 Python-owned。
- TS `packages/amadeus/tools.ts` 只保留 schema bridge 或从 Python `/tools/list` 拉取。
- 加入超时、异常归一化、结果大小限制、工具审计表。
