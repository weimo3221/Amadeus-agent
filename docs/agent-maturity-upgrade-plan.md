...
1. 轻量任务系统，而不是完整 Kanban：
   ...
   - 已完成第二片：简单 in-process worker、queued → running → succeeded/failed/cancelled 真实状态流转、worker claim/heartbeat、运行中 backing turn cooperative cancel，以及模型可调用的 `create_task` / `list_tasks` / `cancel_task` 工具。
   - 下一步：失败重试、worker 重启恢复和 runtime task events 广播。
2. Agent turn 控制：
