...
1. 轻量任务系统，而不是完整 Kanban：
   - 已完成第一片：`tasks` / `task_events` SQLite 存储，`queued` / `running` / `blocked` / `done` / `failed` / `cancelled` 状态，`GET /tasks`、`POST /tasks`、`GET /tasks/{id}/events`、`POST /tasks/{id}/cancel`，TypeScript bridge 代理，以及 Main UI active task 状态面板。
   - 下一步：一个简单 in-process worker、真实状态流转、worker heartbeat、失败重试和 runtime task events 广播。
2. Agent turn 控制：
